// File: src/app/view-only/absensi/alfa/page.tsx
"use client";

import React, { useState, useEffect } from "react";
import Link from "next/link";
import { ArrowLeft, Search, User, AlertTriangle, XCircle, CheckCircle } from "lucide-react";
...
async function fetchAbsensiAlfa(): Promise<Absensi[]> {
  try {
    const res = await fetch(`/api/absensi/alfa`);
    if (!res.ok) {
      throw new Error("Gagal mengambil data absensi alfa");
    }
...
export default function AbsensiAlfaPage() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        setLoading(true);
        const data = await fetchAbsensiAlfa();
        setAbsensiList(data);
        setError(null);
      } catch (err) {
        console.error("Error loading ", err);
        setError("Gagal memuat data absensi alfa.");
      } finally {
        setLoading(false);
      }
    }
  ...
  const totalAlfa = filteredAbsensi.length;
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 bg-white dark:bg-[#1e293b] p-4 sm:p-5 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 gap-4 sm:gap-0">
          <div className="text-center sm:text-left">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <h1 className="text-xl sm:text-2xl font-bold">Detail Absensi: Alfa</h1>
            </div>
            <p className="text-gray-500 dark:text-gray-400 text-sm">
              Kelola data siswa yang absen tanpa keterangan (Alfa).
            </p>
          </div>
          <div className="flex flex-col sm:flex-row gap-3">
            <Link
              href="/view-only/dashboard"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-white dark:bg-[#1e293b] p-4 rounded-xl shadow border border-gray-200 dark:border-gray-700">
            <p className="text-gray-500 dark:text-gray-400 text-sm">Total Alfa Hari Ini</p>
            <p className="text-2xl font-bold text-gray-600 dark:text-gray-400">{totalAlfa}</p>
          </div>
          <div className="bg-white dark:bg-[#1e293b] p-4 rounded-xl shadow border border-gray-200 dark:border-gray-700">
        ...
        {loading ? (
          <p className="text-center text-gray-500 dark:text-gray-400 py-6">
            Memuat data absensi alfa...
          </p>
        ) : filteredAbsensi.length === 0 ? (
          <p className="text-center text-gray-500 dark:text-gray-400 py-6">
            Tidak ada data absensi alfa ditemukan.
          </p>
        ) : (
