...
type AddAgendaFormProps = {
  onAdd: (formData: FormData, id?: string) => void;
...
export default function AddAgendaForm({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!judul.trim() || !tanggal || guruId === "") return;
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-4">
          ...
          <label className="block">
            ...
            />
            {attachment && (
              <span className="text-xs mt-1 text-gray-500 dark:text-gray-400 block">File baru: {attachment.name}</span>
            )}
            {isEditMode && initialData?.doc_path && !attachment && (
                 <span className="text-xs mt-1 text-gray-500 dark:text-gray-400 block">
                    Dokumen lama tersimpan: <a href={initialData.doc_path} target="_blank" rel="noopener noreferrer" className="underline">Lihat</a>
                 </span>
            )}
          </label>
        </div>
        <div className="flex gap-3 pt-3">
          <button
            type="button"
            onClick={onCancel}
            className="flex-1 bg-gray-300 hover:bg-gray-400 text-gray-800 rounded-lg px-4 py-2.5 text-sm transition-colors font-medium shadow-md"
          >
            Batal
          </button>
          <button
            type="submit"
            className="flex-1 bg-blue-600 hover:bg-blue-500 text-white rounded-lg px-4 py-2.5 text-sm transition-colors font-medium shadow-md hover:shadow-lg"
          >
