// app/components/guru-components/calendar/MonthYearSelector.tsx
"use client";
...
export default function MonthYearSelector({
  ...
  const monthNames = Array.from(
    { length: 12 },
    (_, i) => new Date(2000, i, 1).toLocaleString("id-ID", { month: "long" })
  );

  useEffect(() => setLocalYear(yearValue), [yearValue]);
  ...
  return (
    <div className="flex flex-wrap gap-3">
      <div className="bg-gray-100 dark:bg-gray-800 rounded-xl p-3 shadow-sm border border-gray-200 dark:border-gray-700">
        ...
        >
          {monthNames.map((m, i) => (
            <option key={m} value={i} className="bg-white dark:bg-gray-800">{m}</option>
          ))}
        </select>
      </div>
      <div className="bg-gray-100 dark:bg-gray-800 rounded-xl p-3 shadow-sm border border-gray-200 dark:border-gray-700">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center justify-center w-8 h-8 rounded-lg bg-gray-200 hover:bg-gray-300 text-gray-800 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white"
