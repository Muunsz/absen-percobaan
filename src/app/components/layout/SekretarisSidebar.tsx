...
const SidebarSekretaris = ({ children }: { children: React.ReactNode }) => {
  ...
  const menuItems = [
    {
      ...
      items: [
        { href: "/sekretaris/dashboard", label: "Dashboard", icon: Home },
        { href: "/sekretaris/calendar", label: "Kalender", icon: Calendar },
        { href: "/sekretaris/isi-absensi", label: "Isi Absensi", icon: ClipboardList },
        { href: "/sekretaris/rekapan-absen", label: "Daftar Hadir", icon: BookOpen },
      ],
    },
  ];

  const getPageTitle = (pathname: string): string => {
    if (pathname.includes("/sekretaris/dashboard")) return "Dashboard";
    if (pathname.includes("/sekretaris/calendar")) return "Kalender";
    if (pathname.includes("/sekretaris/isi-absensi")) return "Isi Absensi";
    if (pathname.includes("/sekretaris/rekapan-absen")) return "Daftar Hadir";
    return "Halaman";
  };
        ...
        <div className="flex items-center space-x-2">
          ...
          {isSidebarOpen && (
            <div className="flex flex-col">
              <span className="text-sm font-medium">Sekretaris</span>
              <span className="text-xs text-gray-500 dark:text-gray-400">Akun Untuk Mengabsen</span>
            </div>
          )}
        </div>
  ...
  return (
    <div className="flex min-h-screen bg-white dark:bg-gray-900 text-gray-900 dark:text-white transition-colors duration-300">
      ...
      >
        ...
        >
          <Link href="/sekretaris/dashboard" className="flex items-center space-x-2">
            ...
            {isSidebarOpen && <span className="text-xl font-bold">Sekretaris</span>}
          </Link>
        </div>
      ...
      "
          >
            <div className="flex items-center space-x-3">
              ...
              <h1 className="text-lg font-semibold">Sekretaris - {pageTitle}</h1>
            </div>
