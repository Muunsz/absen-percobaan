...
const SidebarView = ({ children }: { children: React.ReactNode }) => {
  ...
  const menuItems = [
    {
      ...
      items: [
        { href: "/view-only/dashboard", label: "Dashboard", icon: Home },
        { href: "/view-only/calendar", label: "Kalender", icon: Calendar },
        { href: "/view-only/rekap-absensi", label: "Rekapan Absensi", icon: ClipboardList },
      ],
    },
  ];

  const getPageTitle = (pathname: string): string => {
    if (pathname.includes("/view-only/dashboard")) return "Dashboard";
    if (pathname.includes("/view-only/calendar")) return "Kalender";
    if (pathname.includes("/view-only/rekap-absensi")) return "Rekapan Absensi";
    return "Halaman";
  };
        ...
        <div className="flex items-center space-x-2">
          ...
          {isSidebarOpen && (
            <div className="flex flex-col">
              ...
              <span className="text-xs text-gray-500 dark:text-gray-400">akses hanya untuk melihat</span>
            </div>
          )}
        </div>
  ...
  return (
    <div className="flex min-h-screen bg-white dark:bg-gray-900 text-gray-900 dark:text-white transition-colors duration-300">
      ...
      >
        ...
        >
          <Link href="/view-only/dashboard" className="flex items-center space-x-2">
            ...
            {isSidebarOpen && <span className="text-xl font-bold">View-Only</span>}
          </Link>
        </div>
        ...
        {isSidebarOpen ? (
          ...
          >
            <div className="flex items-center space-x-3">
              ...
              <h1 className="text-lg font-semibold">View - {pageTitle}</h1>
            </div>
