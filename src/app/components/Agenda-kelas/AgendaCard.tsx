...
export default function AgendaCard({ agenda, onDelete, onEdit, isDeleting }: AgendaCardProps) {
    return (
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 hover:shadow-xl hover:border-blue-500/50 dark:hover:border-blue-500 transition-all shadow-md space-y-4 flex flex-col h-full">
            <div className="flex items-center justify-between border-b border-dashed border-gray-200 dark:border-gray-700 pb-3">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xl font-bold text-gray-900 dark:text-white">{agenda.jam}</span>
                </div>
                <div className="flex gap-2">
                    <button onClick={() => onEdit(agenda)} className="p-1.5 rounded-full text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors group disabled:opacity-50" title="Edit Agenda" disabled={isDeleting}>
                    ...
                    <button
                        ...
                        disabled={isDeleting}
                    >
                        {isDeleting ? <Loader2 className="size-4 animate-spin text-red-500" /> : <Trash2 className="size-4 group-hover:scale-110 transition-transform" />}
                    </button>
                </div>
            </div>
            <h3 className="text-2xl text-gray-900 dark:text-white font-extrabold leading-snug tracking-tight">{agenda.materi}</h3>
            <div className="space-y-2 text-sm pt-2">
                <p className="text-gray-700 dark:text-gray-300 flex items-start gap-2">
                    ...
                    <span className="font-semibold">{agenda.namaGuru}</span>
                </p>
                <p className="text-gray-700 dark:text-gray-300 flex items-start gap-2">
            ...
            {agenda.dokumentasi ? (
                ...
                <p className="text-xs text-gray-400 dark:text-gray-600 mt-auto pt-3 border-t border-gray-200 dark:border-gray-700">Tidak ada tautan dokumentasi.</p>
            )}
        </div>
    )
}