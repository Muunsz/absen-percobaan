// components/calendar/KPISection.tsx
"use client";
import { useMemo } from "react";

type KPISectionProps = {
  // Terima events bulan ini, bukan hanya count
  monthEvents: {
    id_guru: number;
...
export default function KPISection({ monthEvents }: KPISectionProps) {
  // Hitung guru unik yang berpartisipasi
  const uniqueGuru = useMemo(() => {
    const guruSet = new Set<number>();
    monthEvents.forEach(event => {
      if (event.id_guru) {
        guruSet.add(event.id_guru);
      }
    });
    return guruSet.size;
  }, [monthEvents]);

  // Hitung kelas unik yang terlibat (abaikan null/undefined)
  const uniqueKelas = useMemo(() => {
    const kelasSet = new Set<number>();
    monthEvents.forEach(event => {
      if (event.id_kelas != null) { // != null menangkap undefined dan null
        kelasSet.add(event.id_kelas);
      }
    });
    return kelasSet.size;
  }, [monthEvents]);
