...
const jurusanData: Record<string, { color: string; icon: string }> = {
  ...
  "Desain Gambar Mesin": { color: "#ef4444", icon: "/images/Logo/Mesin.webp" },
  "Teknik Kendaraan Ringan": { color: "#fb923c", icon: "/images/Logo/Otomotif.png" },
  "Rekayasa Perangkat Lunak": { color: "#60a5fa", icon: "/images/Logo/PPLG.png" },
  "Teknik Komputer dan Jaringan": { color: "#3b82f6", icon: "/images/Logo/TKJ.png" },
  "Pengembangan Perangkat Lunak dan Gim": { color: "#60a5fa", icon: "/images/Logo/PPLG.png" },
  "Broadcasting dan Perfilman": { color: "#9ca3af", icon: "/images/Logo/BP.png" },
};
...
export default function JurusanPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const res = await fetch(`/api/absensi/${jurusanParam}`);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white">
        <div className="text-center">
          <h2 className="text-xl font-bold">Tidak ada kelas untuk jurusan ini</h2>
          <a
            href="/admin/absensi"
          ...
          {filtered.length === 0 ? (
            ...
            filtered.map((k) => (
              <KelasCard
                ...
                label={k.kelas}
                color={color}
                href={`/admin/absensi/${jurusanParam}/${encodeURIComponent(
                  k.kelas
                )}`}
              />
            ))
