...
// [1] Perbarui tipe User untuk menyertakan nama_lengkap
type User = {
  id: number;
  username: string;
  nama_lengkap?: string | null; // Tambahkan nama_lengkap
  password?: string;
  id_mapel: number;
...
export default function GuruPage() {
  ...
  const [selectedMapelId, setSelectedMapelId] = useState<number | string>("");
  ...
  // Ref untuk modal content agar bisa detect klik di luar modal
  ...
  useEffect(() => {
    const fetchGuruUsers = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await fetch("/api/users/guru");
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!username.trim() || !selectedMapelId) {
      alert("Username dan Mata Pelajaran wajib diisi");
      return;
    }
    ...
    try {
      if (editingId === null) {
        ...
        // UPDATE
        const res = await fetch("/api/edit_account/guru", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
        ...
        if (!res.ok) {
          const err = await res.json();
          throw new Error(err.error || "Gagal memperbarui guru");
        }
  ...
  const handleEdit = (user: User) => {
    ...
    setPassword("");
    setSelectedMapelId(user.id_mapel);
  ...
  const resetForm = () => {
    ...
    setNamaLengkap("");
    setPassword("");
    setSelectedMapelId(options.length > 0 ? options[0].id : "");
    setEditingId(null);
  };
  ...
  const filteredUsers = users.filter(
    (u) =>
      ...
      u.mataPelajaran.toLowerCase().includes(searchTerm.toLowerCase())
  );
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0f172a] dark:text-white transition-colors">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold">Manajemen Pengguna - Guru</h1>
            <p className="text-gray-500 dark:text-gray-400">Kelola data akun guru</p>
          </div>
          <button
        ...
        <div className="relative mb-6">
          ...
          <input
            type="text"
            placeholder="Cari berdasarkan username, nama, atau mapel..."
            value={searchTerm}
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
            ...
            >
              ...
              <h2 className="text-lg font-bold mb-4">
                {editingId !== null ? "Edit Guru" : "Tambah Guru"}
              </h2>
