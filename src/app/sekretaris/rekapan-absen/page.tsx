...
export default function RekapAbsensiPage() {
  ...
  useEffect(() => {
    async function fetchClasses() {
        try {
            const response = await fetch('/api/rekap-absen-s');
      ...
      <div className="max-w-screen-2xl mx-auto space-y-6">
        <ThemedCard className="p-6 flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div className="space-y-1 flex-1">
            <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white">Daftar Hadir Siswa</h1>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Gunakan Pilih Tanggal di bawah untuk menentukan tanggal sebelum melihat detail daftar hadir.
            </p>
          </div>
